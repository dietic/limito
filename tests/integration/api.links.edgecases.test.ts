...
const activeLink = {
  ...
  fallback_url: null, mode: 'by_clicks' as const, click_limit: 10, click_count: 0, last_clicked_at: null,
  expires_at: null, is_active: true, created_at: '2024-01-01T00:00:00.000Z'
}
const expiredLink = { ...activeLink, id: 'l2', slug: 'expired1', is_active: false }
...
describe('api/links edge cases', () => {
  ...
  it('GET /api/links hasMore true when more items exist and counts are returned', async () => {
    ...
    const res = await GET(new Request('http://localhost/api/links?limit=1&offset=0&filter=all') as unknown as NextRequest)
  ...
  it('GET /api/links?filter=expired returns only expired links', async () => {
    ...
    vi.doMock('@/lib/supabase', () => ({ getServiceClient: () => makeSupabaseQueue(queue) }))
    const { GET } = await import('@/app/api/links/route')
    const res = await GET(new Request('http://localhost/api/links?limit=10&offset=0&filter=expired') as unknown as NextRequest)
    const json = await (res as Response).json()
  ...
  it('POST /api/links returns 409 when slug already exists', async () => {
    ...
    const body = { destination_url: 'https://example.com', mode: 'by_clicks', click_limit: 5, slug: 'taken' }
  ...
  it('GET /api/links/[id] returns 404 when not found', async () => {
    ...
    const res = await GET(new Request('http://localhost/api/links/doesnotexist') as unknown as NextRequest, { params: Promise.resolve({ id: 'doesnotexist' }) })
