...
describe('api/links unauthorized errors', () => {
  ...
  it('GET /api/links returns 401 on unauthorized', async () => {
    vi.doMock('@/lib/auth', () => ({ requireAuth: async () => { throw new Error('Unauthorized') } }))
    const { GET } = await import('@/app/api/links/route')
  const res = await GET(makeReq('http://localhost/api/links') as unknown as NextRequest)
  ...
  const res = await GET(makeReq('http://localhost/api/links/123') as unknown as NextRequest, { params: Promise.resolve({ id: '123' }) })
    expect(res.status).toBe(401)
    const json = await (res as Response).json()
    expect(json).toMatchObject({ error: true, message: 'Unauthorized' })
  })
  ...
  const res = await DELETE(makeReq('http://localhost/api/links/123') as unknown as NextRequest, { params: Promise.resolve({ id: '123' }) })
    expect(res.status).toBe(401)
    const json = await (res as Response).json()
    expect(json).toMatchObject({ error: true, message: 'Unauthorized' })
  })
...
describe('api/links validation errors', () => {
  ...
  const res = await POST(makeReq('http://localhost/api/links', { foo: 'bar' }) as unknown as NextRequest)
