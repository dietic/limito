...
describe('api/links/[id] happy paths', () => {
  ...
  it('GET /api/links/[id] returns a link', async () => {
    ...
    const res = await GET(new Request('http://localhost/api/links/l1') as unknown as NextRequest, { params: Promise.resolve({ id: 'l1' }) })
  ...
  it('PATCH /api/links/[id] updates destination_url', async () => {
    ...
    const queue = [ { data: sampleLink, error: null }, { data: updated, error: null } ]
    vi.doMock('@/lib/supabase', () => ({ getServiceClient: () => makeSupabaseQueue(queue) }))
    const { PATCH } = await import('@/app/api/links/[id]/route')
    const body = { destination_url: 'https://limi.to' }
    const res = await PATCH(new Request('http://localhost/api/links/l1', { method: 'PATCH', body: JSON.stringify(body) }) as unknown as NextRequest, { params: Promise.resolve({ id: 'l1' }) })
    expect(res.status).toBe(200)
    const json = await (res as Response).json()
    expect(json.success).toBe(true)
  ...
  it('DELETE /api/links/[id] deletes a link', async () => {
    ...
    const res = await DELETE(new Request('http://localhost/api/links/l1', { method: 'DELETE' }) as unknown as NextRequest, { params: Promise.resolve({ id: 'l1' }) })
    expect(res.status).toBe(200)
    const json = await (res as Response).json()
    expect(json.success).toBe(true)
    expect(json.data.id).toBe('l1')
