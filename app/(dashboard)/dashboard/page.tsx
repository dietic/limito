...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const up =
      typeof window !== "undefined"
        ...
        : null;
    if (!up || upgradeShownRef.current) return;
    if (up === "plus" || up === "pro") {
      upgradeShownRef.current = true;
      toast({
        title: "Subscription updated",
        description: `Your account is now on ${up.toUpperCase()} plan.`,
        variant: "success",
        durationMs: 4000,
      });
      try {
  ...
  // Redirect to login when unauthenticated (avoid conditional hook order by handling in effect)
  useEffect(() => {
