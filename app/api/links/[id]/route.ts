...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { data: existing, error: getErr } = await sb.from('links').select('*').eq('id', params.id).eq('owner_id', userId).single()
    ...
    if (parsed.data.fallback_url !== undefined) {
      const fb = parsed.data.fallback_url ? sanitizeUrl(parsed.data.fallback_url) : null
      if (parsed.data.fallback_url && !fb) return jsonError('Invalid fallback URL', 400)
    ...
    if (parsed.data.expires_at !== undefined) payload['expires_at'] = parsed.data.expires_at
    if (parsed.data.click_limit !== undefined) payload['click_limit'] = parsed.data.click_limit
    if (parsed.data.is_active !== undefined) payload['is_active'] = parsed.data.is_active

    const { data, error } = await sb.from('links').update(payload).eq('id', params.id).eq('owner_id', userId).select('*').single()
