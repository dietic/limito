...
export async function POST(request: NextRequest) {
  ...
  if (!rl.allowed) return jsonError('Daily creation limit reached', { status: 429, headers: { 'Retry-After': '86400' } })
    ...
    const fallback = parsed.data.fallback_url ? sanitizeUrl(parsed.data.fallback_url) : null
    if (parsed.data.fallback_url && !fallback) return jsonError('Invalid fallback URL', 400)
    ...
    const insertPayload = {
      ...
      expires_at: parsed.data.mode === 'by_date' ? parsed.data.expires_at : null,
      click_limit: parsed.data.mode === 'by_clicks' ? parsed.data.click_limit : null,
      click_count: 0,
      last_clicked_at: null,
      is_active: true
    }

    const { data, error } = await sb.from('links').insert(insertPayload).select('*').single()
    if (error) {
