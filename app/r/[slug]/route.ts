...
export async function GET(request: NextRequest, props: { params: Promise<{ slug: string }> }) {
  ...
  if (!perIp.allowed) return new NextResponse('Too Many Requests', { status: 429 })
  const global = await allowAndIncrement('redirect_slug', `slug:${params.slug}`, config.rateLimit.globalPerSlugPerMin, 60_000, sb)
  if (!global.allowed) return new NextResponse('Too Many Requests', { status: 429 })
  const { data: link } = await sb.from('links').select('*').eq('slug', params.slug).single()
  if (!link) {
  ...
  await Promise.all([
    sb.from('links').update({ click_count: newCount, last_clicked_at: nowIso }).eq('id', link.id),
    sb.from('click_events').insert({ link_id: link.id, clicked_at: nowIso, referrer, user_agent: userAgent })
  ])
